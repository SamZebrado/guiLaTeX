...
### 🟡 P2: （可选）正式添加 font_family_en 字段到 Qt 模型
**任务ID**: TASK-032  
**负责人**: Qt  
**状态**: 待分配  
**预估工时**: 2-3 小时
...
#### 功能描述
正式添加 font_family_en 字段到 Qt 模型，并在 UI 中提供选择。

#### 技术方案
- 在 Qt 模型的元素数据结构中添加 font_family_en 字段
- 在属性面板中添加英文字体选择控件
- 确保 font_family_en 字段正确导出到 IR

#### 验收标准
- [ ] Qt 模型中包含 font_family_en 字段
- [ ] UI 支持英文字体选择
- [ ] font_family_en 字段正确导出到 IR

---

### 🟠 P1: 将正式导出按钮切换到 Core 路径
**任务ID**: TASK-033  
**负责人**: Qt  
**状态**: 待分配  
**预估工时**: 2-3 小时
...
#### 功能描述
将 Qt 的正式导出按钮从当前的 IR 导出切换到调用 ExportCore 的完整链路。

#### 技术方案
- 修改 MainWindow 的导出逻辑
- 调用 normalize_qt_model_to_ir(qt_model)
- 调用 export_ir_to_latex(ir_data)
- 保存生成的 LaTeX 文件
- 提供用户反馈

#### 验收标准
- [ ] 正式导出按钮调用 ExportCore 完整链路
- [ ] 能正确生成 LaTeX 文件
- [ ] 有清晰的用户反馈
...
#### 功能描述
添加完整的 Qt -> Core 集成测试，确保整个链路稳定可靠。

#### 技术方案
- 创建测试文件 tests/test_qt_to_core_integration.py
- 测试正常流程：Qt 模型 -> normalize -> export -> LaTeX
- 测试边界情况：空模型、单一元素、多元素等
- 测试字段映射正确性
