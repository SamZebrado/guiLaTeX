...
#### 功能描述
完善 PDF 导出流程，通过 Core->tex->编译得到 PDF，而不是直接复制现有 PDF。

#### 技术方案
- 修改 export_document 方法，使用 Core 导出 LaTeX
- 调用 LaTeX 引擎编译生成 PDF
- 确保 PDF 导出路径清晰

#### 验收标准
- [ ] PDF 导出通过 Core->tex->编译得到
- [ ] 生成的 PDF 文件正确
- [ ] 导出路径清晰
...
#### 功能描述
完善变换菜单下的具体功能，包括 x / y / 宽 / 高 / 旋转 / 图层编号的直接操作。
...
#### 功能描述
解决环境问题，确保 PyQt6 依赖正确安装，自动化测试能够运行。
