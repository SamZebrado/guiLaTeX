#!/usr/bin/env python3
"""
guiLaTeX Web 线 - 严格 browser rerun 取证脚本

功能：
1. 使用 Playwright 真正从浏览器导出 fresh IR
2. 记录详细的执行过程和时间戳
3. 保存页面截图作为证据
4. 重跑至少一条旧 regression
5. 运行 bridge 生成 .tex
6. 对 .tex 做最小合法性检查
7. 保存证据到带时间戳的新鲜证据目录
"""
...
# 新鲜证据目录
FRESH_EVIDENCE_DIR = os.path.join(PROJECT_ROOT, 'docs', 'contest_evidence', f'web_to_core_bridge_forensics_{TIMESTAMP}')
# Web 原型目录
WEB_PROTOTYPE_DIR = os.path.join(PROJECT_ROOT, 'web_prototype')

# 输出文件路径
FRESH_IR_PATH = os.path.join(WEB_PROTOTYPE_DIR, f'web_browser_exported_ir_{TIMESTAMP}.json')
TEX_OUTPUT_PATH = os.path.join(FRESH_EVIDENCE_DIR, f'web_browser_export_output_{TIMESTAMP}.tex')
EXECUTION_LOG = os.path.join(FRESH_EVIDENCE_DIR, f'execution_log_{TIMESTAMP}.txt')
SCREENSHOT_PATH = os.path.join(FRESH_EVIDENCE_DIR, f'page_screenshot_{TIMESTAMP}.png')
...
log("====================================")
log("guiLaTeX Web 线 - 严格 browser rerun 取证")
log(f"时间戳: {TIMESTAMP}")
log("====================================")
...
async function exportFreshIR() {{
    ...
    try {{
        ...
        console.log('IR 结构:', JSON.stringify(freshIR, null, 2));
        
        // 在 IR 中添加导出时间戳
        freshIR.export_timestamp = new Date().toISOString();
        freshIR.export_round = '{TIMESTAMP}';
...
""".replace('{WEB_PROTOTYPE_DIR}', WEB_PROTOTYPE_DIR).replace('{FRESH_IR_PATH}', FRESH_IR_PATH).replace('{SCREENSHOT_PATH}', SCREENSHOT_PATH).replace('{TIMESTAMP}', TIMESTAMP);
    
    # 保存临时 Playwright 脚本
    temp_script = os.path.join(WEB_PROTOTYPE_DIR, f'playwright_forensics_{TIMESTAMP}.js')
    ...
    if result.returncode == 0 and os.path.exists(FRESH_IR_PATH):
        ...
        log(f"✅ 导出时间戳: {ir_data.get('export_timestamp')}")
        log(f"✅ 导出轮次: {ir_data.get('export_round')}")
...
# 复制必要的文件到证据目录
files_to_copy = [
    (FRESH_IR_PATH, os.path.join(FRESH_EVIDENCE_DIR, f'web_browser_exported_ir_{TIMESTAMP}.json')),
    (TEX_OUTPUT_PATH, os.path.join(FRESH_EVIDENCE_DIR, f'web_browser_export_output_{TIMESTAMP}.tex')),
    (EXECUTION_LOG, os.path.join(FRESH_EVIDENCE_DIR, f'execution_log_{TIMESTAMP}.txt')),
    (SCREENSHOT_PATH, os.path.join(FRESH_EVIDENCE_DIR, f'page_screenshot_{TIMESTAMP}.png')),
    (__file__, os.path.join(FRESH_EVIDENCE_DIR, 'run_browser_rerun_forensics.py')),
