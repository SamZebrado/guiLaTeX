...
# 新鲜证据目录
FRESH_EVIDENCE_DIR = os.path.join(PROJECT_ROOT, 'docs', 'contest_evidence', f'web_to_core_bridge_real_{TIMESTAMP}')
# Web 原型目录
WEB_PROTOTYPE_DIR = os.path.join(PROJECT_ROOT, 'web_prototype')

# 输出文件路径
FRESH_IR_PATH = os.path.join(WEB_PROTOTYPE_DIR, f'web_real_exported_ir_{TIMESTAMP}.json')
...
""".replace('{WEB_PROTOTYPE_DIR}', WEB_PROTOTYPE_DIR).replace('{FRESH_IR_PATH}', FRESH_IR_PATH).replace('{SCREENSHOT_PATH}', SCREENSHOT_PATH).replace('{TIMESTAMP}', TIMESTAMP)
    ...
    # 保存临时 Playwright 脚本
    temp_script = os.path.join(WEB_PROTOTYPE_DIR, f'playwright_real_export_{TIMESTAMP}.js')
...
# 复制必要的文件到证据目录
files_to_copy = [
    (FRESH_IR_PATH, os.path.join(FRESH_EVIDENCE_DIR, f'web_real_exported_ir_{TIMESTAMP}.json')),
    (TEX_OUTPUT_PATH, os.path.join(FRESH_EVIDENCE_DIR, f'web_real_export_output_{TIMESTAMP}.tex')),
    (EXECUTION_LOG, os.path.join(FRESH_EVIDENCE_DIR, f'execution_log_{TIMESTAMP}.txt')),
    (SCREENSHOT_PATH, os.path.join(FRESH_EVIDENCE_DIR, f'page_screenshot_{TIMESTAMP}.png')),
    (__file__, os.path.join(FRESH_EVIDENCE_DIR, 'run_real_browser_export.py')),
